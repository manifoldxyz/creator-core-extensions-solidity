...
- 2026-07-18 — manifold-packs: **airdrop now counts toward supply + pause defaults ON (PR #119, branch cxrds-pack-contract).** TWO owner-requested changes reversing earlier design calls: (1) **`airdrop` counts toward `mintedCards`** (lazy-claim parity, `ERC1155LazyPayableClaimCore.airdrop`): bumps the minted total by `sum(amounts)` and, if `maxCardsSupply != 0` and would be exceeded, RAISES the cap to the new total so an airdrop is never blocked — making `maxCardsSupply` a real TOTAL-supply cap (rips + airdrops), closing review finding M1. Trade-off: an airdrop that raises the cap to exactly `mintedCards` leaves zero rip headroom until the owner raises the cap. (2) **`transfersPaused` now defaults to `true`** (paused on deploy, matching upstream `ERC721SeaDropPausable`) — the owner opens the secondary market with `updateTransfersPaused(false)`. The secondary-only SCOPE divergence (`from != 0 && to != 0`) is unchanged, so mint and the rip burn still flow even paused-on-deploy. 119/119 manifoldpacks tests green (airdrop tests rewritten for the new accounting + cap-raise; pausable default tests flipped; 3 rip-flow tests unpause before their setup transfer); runtime size 23,180 B (< 24,576).
