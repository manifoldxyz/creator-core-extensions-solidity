...
- [[manifold-packs]] — Gasless "rip": a dual-role `ERC721SeaDrop` pack collection that is ALSO an extension on a stock ERC1155 cards core; a signer submits collector-signed EIP-712 `RipPermit`s, each burning one pack to mint its committed cards. **Gotcha:** each pack's contents are bound to an on-chain **Merkle `contentsRoot`** (leaf = `keccak256(abi.encode(packId, cardIds, amounts, salt))`, single-hashed + OZ sorted-pair — a double-hash/unsorted off-chain tree bricks every rip with `ContentsMismatch`); the root is **owner-mutable (no pre-mint lock)** and binds the *signer*, not the *owner*. Also: **no nonces** — the pack burn IS the replay lock; init error is `CardsAlreadyInitialized` (dodges the inherited `ConstructorInitializable.AlreadyInitialized()` collision).
