...
- **`updateTransfersPaused(bool) external onlyOwner`** — pause/unpause SECONDARY pack transfers (owner "stop trading" switch). While paused, `transferFrom`/`safeTransferFrom` between non-zero addresses, `approve`, and `setApprovalForAll` revert `TransfersPaused`; **mint and the rip burn are never blocked**. Adapts OpenSea's `ERC721SeaDropPausable`: **defaults to `true`/paused on deploy** (matches upstream — the owner opens the secondary market with `updateTransfersPaused(false)` when ready; because the pause never blocks mint or the rip burn, a paused-on-deploy contract still mints the primary drop and lets collectors rip). The one deliberate divergence is scope: the pause is **limited to secondary moves** via the `_beforeTokenTransfers` `from != 0 && to != 0` guard (upstream reverts on any `from != 0`, blocking burns too). Emits `TransfersPausedChanged`. ^[manifold/contracts/manifoldpacks/ManifoldPacksSeaDropShim.sol#L247]
- **`airdrop(address[] calldata recipients, uint256[] calldata cardIds, uint256[] calldata amounts) external onlyOwner nonReentrant`** — owner escape hatch that mints reserved card variations **directly**, bypassing the pack-burn rip flow (corrections, giveaways, partner allocations). Parallel arrays: `recipients[i]` gets `amounts[i]` of `cardIds[i]`; all three lengths must be equal and non-zero, every `cardIds[i]` must be in the reserved range (else `InvalidCardIds`), reverts `CardsNotInitialized` before init and `InvalidAirdrop` on empty/mismatched arrays. Emits `Airdropped`. **Counts toward the minted total** (lazy-claim parity, `ERC1155LazyPayableClaimCore.airdrop`): bumps `mintedCards` by `sum(amounts)` and, if `maxCardsSupply != 0` and would be exceeded, RAISES the cap to the new total so an airdrop is never blocked. So `maxCardsSupply` is a real TOTAL-supply cap (rips + airdrops), not a rip-only budget. **Trade-off:** an airdrop that raises the cap to exactly `mintedCards` leaves zero headroom — the next rip reverts `MaxCardsSupplyExceeded` until the owner raises `maxCardsSupply` via `updateConfig`. No rip-window gate. ^[manifold/contracts/manifoldpacks/ManifoldPacksSeaDropShim.sol#L298]
...
- **The transfer pause is SECONDARY-only and defaults ON (paused) — do not model its SCOPE on upstream `ERC721SeaDropPausable`.** `transfersPaused` defaults to `true` (paused on deploy, matching upstream — the owner opens trading with `updateTransfersPaused(false)`). The load-bearing divergence is SCOPE, not default: `_beforeTokenTransfers` gates on `from != 0 && to != 0` (upstream reverts on any `from != 0`). Without the `to != 0` carve-out an active pause would revert the rip's `_burn` and brick every collector's rip, and without the `from != 0` carve-out it would block mint. So even paused-on-deploy, mint and rip always flow; only wallet-to-wallet / marketplace moves are pausable. ^[manifold/contracts/manifoldpacks/ManifoldPacksSeaDropShim.sol#L505]
