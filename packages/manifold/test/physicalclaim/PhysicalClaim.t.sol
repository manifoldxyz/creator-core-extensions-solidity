...
contract PhysicalClaimTest is Test {
  ...
  function testNoBurnSpec() public {
    vm.startPrank(owner);
    ...
    burnItems[0] = IPhysicalClaimCore.BurnItem({
      validationType: IPhysicalClaimCore.ValidationType.CONTRACT,
      contractAddress: address(creatorCore721),
      burnTokenSpec: IPhysicalClaimCore.BurnTokenSpec.ERC721_NO_BURN, // no burn!
      burnFunctionSpec: IPhysicalClaimCore.BurnFunctionSpec.MANIFOLD,
      amount: 1,
      minTokenId: 1,
      maxTokenId: 3,
    ...
    // Does not need to approve token for burning
    // creatorCore721.approve(address(example), 1);
    // creatorCore721.approve(address(example), 2);
    ...
    // Verify that the token still exists..
    assertEq(creatorCore721.ownerOf(1), other);
  ...
  function testNoBurnSpecInvalidSender() public {
    vm.startPrank(owner);
  ...
  function testNoGate() public {
    ...
    IPhysicalClaimCore.BurnGroup[] memory burnSet = new IPhysicalClaimCore.BurnGroup[](0);
    IPhysicalClaimCore.VariationLimit[] memory variations = new IPhysicalClaimCore.VariationLimit[](1);
    variations[0] = IPhysicalClaimCore.VariationLimit({
      id: 1,
      totalSupply: 10
    ...
    IPhysicalClaimCore.BurnToken[] memory burnTokens = new IPhysicalClaimCore.BurnToken[](0);
  ...
  function testMakeVariationInactive() public {
    vm.startPrank(owner);
    ...
    // Change variation id to "2"
    vm.stopPrank();
    vm.startPrank(owner);

    variations[0].id = 2;
    variations[0].totalSupply = 69;
    claimPs.variationLimits = variations;
    example.updatePhysicalClaim(instanceId, claimPs);

    // Variation 1 should be inactive now...
    IPhysicalClaimCore.VariationState memory variationStateReturn = example.getVariationState(instanceId, 1);
    assertEq(variationStateReturn.totalSupply, 10);
    assertEq(variationStateReturn.redeemedCount, 1);
    assertEq(variationStateReturn.active, false);

    variationStateReturn = example.getVariationState(instanceId, 2);
    assertEq(variationStateReturn.totalSupply, 69);
    assertEq(variationStateReturn.redeemedCount, 0);
    assertEq(variationStateReturn.active, true);
    ...
    // Cannot redeem for variation one...
    // Burn another
    burnTokens[0].id = 2;
    submissions[0].currentClaimCount = 1;

    // Should fail
    vm.expectRevert(IPhysicalClaimCore.InvalidVariation.selector);
    example.burnRedeem(submissions);
