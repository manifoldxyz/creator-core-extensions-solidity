...
contract ERC721LazyPayableClaimMetadataV2 is IERC721LazyPayableClaimMetadata {
    using Strings for uint256;

    function tokenURI(address creatorContract, uint256 tokenId, uint256 instanceId, uint24 mintOrder)
        external
        pure
        override
...
contract ERC721LazyPayableClaimV2Test is Test {
    ...
    function setUp() public {
        ...
        example = new ERC721LazyPayableClaimV2(owner, address(delegationRegistry), address(delegationRegistryV2));
        // set mint fees
    ...
    function testAccess() public {
        ...
        vm.expectRevert();
        example.updateTokenURIParams(address(creatorCore), 1, ILazyPayableClaimCore.StorageProtocol.IPFS, true, "");
        ...
        example.updateTokenURIParams(
            address(creatorCore), 1, ILazyPayableClaimCore.StorageProtocol.ARWEAVE, true, "arweaveHash3"
        );
        assertEq("https://arweave.net/arweaveHash3", creatorCore.tokenURI(1));
        // Extend uri
        vm.expectRevert();
        example.extendTokenURI(address(creatorCore), 1, "");
        example.updateTokenURIParams(address(creatorCore), 1, ILazyPayableClaimCore.StorageProtocol.NONE, true, "part1");
    ...
    function testUpdateClaimSanitization() public {
        ...
        vm.expectRevert(ILazyPayableClaimCore.InvalidStorageProtocol.selector);
        example.updateTokenURIParams(address(creatorCore), 1, ILazyPayableClaimCore.StorageProtocol.INVALID, true, "");

        vm.expectRevert(ILazyPayableClaimCore.InvalidStorageProtocol.selector);
        example.updateTokenURIParams(address(creatorCore), 1, ILazyPayableClaimCore.StorageProtocol.ADDRESS, true, "");
    ...
    function testFunctionality() public {
        ...
        (uint256 instanceId, IERC721LazyPayableClaim.Claim memory claimInfo) =
