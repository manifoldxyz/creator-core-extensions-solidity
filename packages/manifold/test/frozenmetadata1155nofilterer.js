...
contract('Manifold 1155 Frozen Metadata No Filterer', function ([minter, ...accounts]) {
  const name = 'Token';
  const symbol = 'NFT';
  const [
    deployer,
  ...
  describe('Manifold 1155 Frozen Metadata No Filterer', function() {
    var creator;
    var extension;
    ...
    it('transfer restriction test', async function () {
      await extension.mintTokenNew(creator.address, [operator], [1], ["{hey:hey}"], {from:owner})
      ...
      await creator.safeTransferFrom(operator, another, 1, 1, "0x0", {from:operator});
    });
