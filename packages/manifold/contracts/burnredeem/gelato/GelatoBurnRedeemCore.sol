...
/**
 * @title Gelato-Enabled Burn Redeem Core
 * @author manifold.xyz
 * @notice Core logic for Burn Redeem with Gelato Relay support (ERC-2771)
 *
 * KEY CHANGE: Uses _msgSender() instead of msg.sender throughout to support meta-transactions
 * This allows users to:
 * 1. Sign an approval message off-chain (no gas)
 * 2. Sign a burn transaction message (no gas)
 * 3. Gelato relayer submits the transaction
 * 4. User pays gas fees from msg.value using callWithSyncFeeERC2771
 * 5. No sponsorship burden for developers
 *
 * How Gelato Relay Works:
 * - User signs EIP-712 structured data off-chain
 * - Gelato's trusted forwarder (0xaBcC9b596420A9E9172FD5938620E265a0f9Df92) submits tx
 * - GelatoRelayContext decodes the original sender from calldata
 * - _msgSender() returns the actual user, not the relayer
 * - Gas fees are automatically deducted from msg.value and sent to relayer
 */
abstract contract GelatoBurnRedeemCore is
    ERC165,
    AdminControl,
    ReentrancyGuard,
    GelatoRelayContext,
    ...
    ICreatorExtensionTokenURI
{
    ...
    /**
     * @notice CRITICAL: Override _msgSender to use Gelato's context when called via relay
     * @dev This is what enables the single-transaction UX
     * - When called directly: returns msg.sender
     * - When called via Gelato relay: extracts original signer from calldata
     */
    function _msgSender() internal view override(Context, GelatoRelayContext) returns (address) {
        return GelatoRelayContext._msgSender();
    }

    /**
     * @notice Override _msgData to use Gelato's context when called via relay
     * @dev Removes appended sender address from calldata when called via trusted forwarder
     */
    function _msgData() internal view override(Context, GelatoRelayContext) returns (bytes calldata) {
        return GelatoRelayContext._msgData();
    }
     ...
     * that a burn redeem's initializer is an admin on the creator contract
     * @param creatorContractAddress    the address of the creator contract to check the admin against
     */
    function _validateAdmin(address creatorContractAddress) internal view {
        // IMPORTANT: Uses _msgSender() to get actual caller (not relayer)
        if (!IAdminControl(creatorContractAddress).isAdmin(_msgSender())) {
            revert NotAdmin(creatorContractAddress);
        }
    }

    /**
     * Initializes a burn redeem with base parameters
     */
     ...
     * @dev IMPORTANT: Uses _msgSender() to get actual user when called via Gelato relay
     */
    function burnRedeem(address creatorContractAddress, uint256 instanceId, uint32 burnRedeemCount, BurnToken[] calldata burnTokens) external payable override nonReentrant {
        // _msgSender() returns actual user (not relayer)
        address user = _msgSender();

        uint256 payableCost = _burnRedeem(msg.value, creatorContractAddress, instanceId, burnRedeemCount, burnTokens, _isActiveMember(user), true, "", user);

        // When using Gelato's callWithSyncFeeERC2771, the relay fee is automatically
        // deducted from msg.value before this contract receives it.
        // Any excess is refunded to the user.
        if (msg.value > payableCost) {
            _forwardValue(payable(user), msg.value - payableCost);
        }
    }

    /**
     * (Batch overload) see {IBurnRedeemCore-burnRedeem}.
     * @dev IMPORTANT: Uses _msgSender() for actual user
     */
    function burnRedeem(address[] calldata creatorContractAddresses, uint256[] calldata instanceIds, uint32[] calldata burnRedeemCounts, BurnToken[][] calldata burnTokens) external payable override nonReentrant {
        ...
        address user = _msgSender();
        bool isActiveMember = _isActiveMember(user);
        uint256 msgValueRemaining = msg.value;

        for (uint256 i; i < creatorContractAddresses.length;) {
            msgValueRemaining -= _burnRedeem(msgValueRemaining, creatorContractAddresses[i], instanceIds[i], burnRedeemCounts[i], burnTokens[i], isActiveMember, false, "", user);
            unchecked { ++i; }
        }

        if (msgValueRemaining != 0) {
            _forwardValue(payable(user), msgValueRemaining);
        }
    }
    ...
    function burnRedeemWithData(address creatorContractAddress, uint256 instanceId, uint32 burnRedeemCount, BurnToken[] calldata burnTokens, bytes calldata data) external payable override nonReentrant {
        ...
        uint256 payableCost = _burnRedeem(msg.value, creatorContractAddress, instanceId, burnRedeemCount, burnTokens, _isActiveMember(user), true, data, user);
        if (msg.value > payableCost) {
            _forwardValue(payable(user), msg.value - payableCost);
        }
    }
    ...
    /**
     * @dev Internal burn redeem logic
     * @param user The actual user performing the burn (from _msgSender())
     */
    function _burnRedeem(
        uint256 msgValue,
        address creatorContractAddress,
        uint256 instanceId,
        ...
        // Do burn redeem - pass actual user address
        _burnTokens(burnRedeemInstance, burnTokens, burnRedeemCount, user, data);
        _redeem(creatorContractAddress, instanceId, burnRedeemInstance, user, burnRedeemCount, data);
     ...
     * @dev IMPORTANT: 'from' is the actual user, msg.sender is the NFT contract
     */
     ...
     * @param owner The actual token owner (from _msgSender())
     */
