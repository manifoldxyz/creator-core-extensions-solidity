...
abstract contract LazyPayableClaim is ILazyPayableClaim, AdminControl {
  ...
  function _transferFunds(
     ...
     */
    if (
      MEMBERSHIP_ADDRESS == ADDRESS_ZERO ||
      !allowMembership ||
  ...
  function _validateMint(
    ...
    if ((startDate > block.timestamp) || (endDate > 0 && endDate < block.timestamp))
      revert ILazyPayableClaim.ClaimInactive();
  ...
  function _checkSignatureAndUpdate(
    ...
    bytes32 expectedMessage = keccak256(
      abi.encodePacked(creatorContractAddress, instanceId, nonce, mintFor, expiration, mintCount)
    );
