...
contract ERC721LazyPayableClaim is IERC165, IERC721LazyPayableClaim, ICreatorExtensionTokenURI, LazyPayableClaim {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(AdminControl, IERC165) returns (bool) {
    return
      interfaceId == type(IERC721LazyPayableClaim).interfaceId ||
  ...
  function airdrop(
    ...
    if (claim.contractVersion >= 3) {
      ...
      for (uint256 i; i < recipients.length; ) {
        // Airdrop the tokens
        uint256[] memory newTokenIds = IERC721CreatorCore(creatorContractAddress).mintExtensionBatch(
          recipients[i],
          amounts[i]
        );
  ...
  function tokenURI(address creatorContractAddress, uint256 tokenId) external view override returns (string memory uri) {
    ...
    if (claim.storageProtocol == StorageProtocol.ADDRESS) {
      return
        IERC721LazyPayableClaimMetadata(_bytesToAddress(bytes(claim.location))).tokenURI(
          creatorContractAddress,
          tokenId,
          instanceId,
          mintOrder
