...
contract ERC1155LazyPayableClaim is IERC165, IERC1155LazyPayableClaim, ICreatorExtensionTokenURI, LazyPayableClaim {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165, AdminControl) returns (bool) {
    return
      interfaceId == type(IERC1155LazyPayableClaim).interfaceId ||
  ...
  ) LazyPayableClaim(initialOwner, delegationRegistry, delegationRegistryV2, mintFee, mintFeeMerkle) {}
  ...
  function initializeClaim(
    ...
    if (claimParameters.storageProtocol == StorageProtocol.ADDRESS && bytes(claimParameters.location).length != 20)
      revert ILazyPayableClaim.InvalidStorageProtocol();
    if (claimParameters.endDate != 0 && claimParameters.startDate >= claimParameters.endDate)
      revert ILazyPayableClaim.InvalidStartDate();
    require(
   ...
   */
  function getClaim(address creatorContractAddress, uint256 instanceId) public view override returns (Claim memory claim) {
  ...
  function mint(
    ...
    if (((++claim.total > claim.totalMax && claim.totalMax != 0) || claim.total > MAX_UINT_24))
      revert ILazyPayableClaim.TooManyRequested();
  ...
  function tokenURI(address creatorContractAddress, uint256 tokenId) external view override returns (string memory uri) {
    ...
    if (claim.storageProtocol == StorageProtocol.ADDRESS) {
      return
        IERC1155LazyPayableClaimMetadata(_bytesToAddress(bytes(claim.location))).tokenURI(
          creatorContractAddress,
          tokenId,
          instanceId
        );
