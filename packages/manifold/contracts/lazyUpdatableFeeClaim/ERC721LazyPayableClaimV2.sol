...
import "./IERC721LazyPayableClaimV2.sol";
import "./IERC721LazyPayableClaimMetadataV2.sol";
 ...
 */
contract ERC721LazyPayableClaimV2 is IERC165, IERC721LazyPayableClaimV2, ICreatorExtensionTokenURI, LazyPayableClaimV2 {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(AdminControl, IERC165) returns (bool) {
        return interfaceId == type(IERC721LazyPayableClaimV2).interfaceId ||
    ...
    function initializeClaim(
        ...
        if (instanceId == 0 || instanceId > MAX_UINT_56) revert ILazyPayableClaimV2.InvalidInstance();
        // revert ILazyPayableClaimV2.if claim at instanceId already exists
    ...
    function extendTokenURI(
        ...
        Claim storage claim = _getClaim(creatorContractAddress, instanceId);
        if (claim.storageProtocol != StorageProtocol.NONE || !claim.identical) revert ILazyPayableClaimV2.InvalidStorageProtocol();
        claim.location = string(abi.encodePacked(claim.location, locationChunk));
        emit ClaimUpdated(creatorContractAddress, instanceId);
    }

    /**
     * See {ILazyPayableClaimV2-getClaim}.
     */
    ...
    /**
     * See {ILazyPayableClaimV2-getClaimForToken}.
     */
    ...
    function mintBatch(address creatorContractAddress, uint256 instanceId, uint16 mintCount, uint32[] calldata mintIndices, bytes32[][] calldata merkleProofs, address mintFor) external payable override {
        ...
        if (((claim.totalMax != 0 && claim.total > claim.totalMax) || claim.total > MAX_UINT_24)) revert ILazyPayableClaimV2.TooManyRequested();
    ...
    function airdrop(address creatorContractAddress, uint256 instanceId, address[] calldata recipients,
        ...
        if (newMintIndex - claim.total - 1 > MAX_UINT_24) revert ILazyPayableClaimV2.TooManyRequested();
    ...
    function tokenURI(address creatorContractAddress, uint256 tokenId) external override view returns(string memory uri) {
        ...
        if (claim.storageProtocol == StorageProtocol.ADDRESS) {
            return IERC721LazyPayableClaimMetadataV2(_bytesToAddress(bytes(claim.location))).tokenURI(creatorContractAddress, tokenId, instanceId, mintOrder);
        }
